 ...
 */
public class BluetoothChatServiceUtil {
    // Debugging
    private static final String TAG = "BTServiceUtil";
     ...
     */
    public BluetoothChatServiceUtil(Context context, Handler handler) {
    ...
    private class AcceptThread extends Thread {
        ...
        public void run() {
            ...
            while (mState != STATE_CONNECTED) {
                ...
                if (socket != null) {
                    synchronized (BluetoothChatServiceUtil.this) {
                        switch (mState) {
                            case STATE_LISTEN:
    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                BluetoothChatServiceUtil.this.start();
                return;
            }
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            byte[] buffer;// 62 bytes per package

            // Keep listening to the InputStream while connected
            while (true) {
                try {
                    buffer = new byte[1024];        // Init buffer every time
                    int bytesCount = mmInStream.read(buffer);// Read from the InputStream
                    byte[] data = new byte[bytesCount];
                    System.arraycopy(buffer, 0, data, 0, bytesCount);
                    mHandler.obtainMessage(BluetoothChatActivity.MESSAGE_READ, data).sendToTarget();
                } catch (IOException e) {
